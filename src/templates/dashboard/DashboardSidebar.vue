<template>
  ...
  >
    ...
    >
      ...
      <a
        href=""
        class="flex items-center gap-2.5 rounded-xl bg-fce40d opacity-80 transition-opacity duration-200 ease-in-out"
      >
        <img
    ...
    >
      ...
      >
        <!-- submenu-header -->
        <div class="flex flex-col-reverse sm:flex-col gap-4 w-full">
          <!-- title -->
        ...
        <!-- Submenu items -->
        <div
          class="flex flex-col justify-center overflow-auto items-center w-full gap-2 py-2"
        >
          <button
            ...
            :class="[
              'relative flex items-center justify-center w-full gap-3 rounded-md px-4 py-2 group transition-all duration-200',
              child.enabled
    ...
    @update:modelValue="val => { closeAllPopups(); isProfileOpen = val }"
    />
    <NotificationPopup
    @update:modelValue="val => { closeAllPopups(); isNotificationOpen = val }"
...
export default {
  ...
  methods: {
    ...
    renderSidebarMenu() {
      ...
      const logoHeight = this.$el.querySelector(
        "a.flex.items-center.gap-2\\.5"
      ).offsetHeight;
    ...
    createOverflowMenu(menuEl, overflowItems) {
      ...
      flyoutWrapper.className =
        "fixed top-[-9999px] left-[-9999px] pointer-events-none z-[9999]";
      ...
      overflowItems.forEach((item) => {
        ...
        `;
        o.className =
          "sidebar-menu-item block transition-all duration-200 ease-in-out rounded-md flex-col items-center justify-center self-stretch w-[4.625rem] h-14 cursor-pointer p-2 whitespace-nowrap text-sm hover:bg-gray-100 rounded transition-colors flex items-center";
        if (!item.enabled) {
          o.className += " opacity-50 pointer-events-none grayscale";
        }
